...
switch (env) {
  ...
  default:
    ...
    allowedHosts.push("papercutting-drily-braylee.ngrok-free.dev"); 
}
...
app.post("/mcp", async (req, res) => {
  try {
    const server = new McpServer(
      {
        name: "Clinical-to-Patient Bridge",
        version: "1.0.0",
      },
      {
        capabilities: {
          extensions: {
            "ai.promptopinion/fhir-context": {
              scopes: [
                ...
                {
                  // Inpatient administered medications (IV drips, etc.)
                  // Required by FetchMedicationsTool
                ...
                {
                  // Required by FetchConditionsAndReportsTool (Empathy Engine)
                  // and GenerateDischargeBriefTool (full-pipeline fallback)
